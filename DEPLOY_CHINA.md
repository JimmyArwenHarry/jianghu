...
## 方案 0：本地电脑试玩（¥0，不买服务器，内网穿透）

不想买任何东西，只想先拉几个朋友在微信里玩一把 → 用 **cpolar 内网穿透**，把你电脑上的江湖录映射成一个公网 https 链接：

1. 注册免费账号：https://www.cpolar.com （邮箱即可，不用实名）
2. 登录后复制 Authtoken（"验证"页），执行一次：
   ```bash
   cpolar authtoken 你的Authtoken
   ```
3. 跑仓库自带的一键脚本（自动构建 + 防休眠 + 起服务 + 开隧道）：
   ```bash
   bash tunnel.sh
   ```
4. 复制脚本打印的 `https://xxxx.cpolar.top` 链接，发微信即可点开即玩。

⚠️ 适用边界：

- **免费版公网地址每 24 小时 / 重启会更换**，每次分享请重新跑一遍 `bash tunnel.sh` 复制新链接
- 电脑要**插电 + 保持开盖**（脚本已自动防休眠，但合盖睡眠会断网）
- 适合"拉朋友玩一把"；想长期稳定、随时能玩，还是用下面 ¥50~130/年的服务器方案
